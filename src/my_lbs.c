...
typedef enum 
{
	NotificationsIndicationsDisabled=0,
	NotificationsEnabled = BT_GATT_CCC_NOTIFY,
	IndicationsEnabled = BT_GATT_CCC_INDICATE,
} ccc_cfg_state_enum_t;

static volatile uint8_t cfgState = NotificationsIndicationsDisabled;
				  ...
				  uint16_t value)
{
	...
	{
			cfgState = IndicationsEnabled;
			LOG_DBG("Button state indications enabled ");
	}
	else if (value == BT_GATT_CCC_NOTIFY)
	{
		cfgState = NotificationsEnabled;
		LOG_DBG("Button state notifications enabled ");
	}
	else 
	{
		cfgState = NotificationsIndicationsDisabled;
		LOG_DBG("Button state notifications and indications DISABLED");
	}
...
int my_lbs_send_button_state_indicate_notify(bool button_state)
{
	if(cfgState == NotificationsEnabled)
	{
		int err = bt_gatt_notify(NULL, &my_lbs_svc.attrs[2], &button_state, sizeof(button_state));
		if(err)
			LOG_ERR("Couldn't notify button state, reason %d", err);
			return err;
	}

	else if (cfgState == IndicationsEnabled)
	{
